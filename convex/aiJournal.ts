...
## 핵심 원칙
- 매출/매입은 세금계산서 발행 시 별도 기록된다
- 입금(고객 대금 수령)은 외상매출금(108) 회수이지, 새로운 매출이 아니다
- 출금(거래처 대금 지급)은 외상매입금(201) 상환이다

## 규칙
1. 반드시 차변 합계 = 대변 합계 (대차균형)
2. 고객 입금: 보통예금(102) 차변 / 외상매출금(108) 대변 (부가세 분리 안함)
3. 거래처 대금 지급: 외상매입금(201) 차변 / 보통예금(102) 대변 (부가세 분리 안함)
4. 직접 비용 출금은 과세 시 부가세 10% 분리: 부가세대급금(113) 차변
5. 면세 거래: 급여, 이자, 4대보험, 보험료 → 부가세 분리 안함
6. 입금 = 보통예금(102) 차변 / 출금 = 보통예금(102) 대변
7. 거래처명이 입력 내용에 포함되면 등록 거래처에서 매칭
8. 100만원 이상 장비/비품 구매는 비품(156)으로, 미만은 소모품비(524)로 처리
9. 절대로 입금을 매출(401/402/403/404) 계정으로 분류하지 마라
10. 결과는 아래 JSON 형식으로만 응답 (마크다운/설명 없이):
