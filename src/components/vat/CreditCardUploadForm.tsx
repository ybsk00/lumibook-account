...
function detectPeriodLabel(period: string): string {
  if (!period) return "";
...
export function CreditCardUploadForm() {
  ...
  const parsePdf = useAction(api.creditCardUploadAi.parseCreditCardPdf);
  const batchCreate = useMutation(api.creditCardUpload.batchCreateWithJournals);
  ...
  const handleFileAdd = useCallback(async (files: FileList | File[]) => {
    ...
    if (pdfFiles.length === 0) {
      setError("PDF 파일(.pdf)만 업로드 가능합니다.");
      return;
    }
    ...
    try {
      ...
      const parsed = await parsePdf({ base64Pdf: base64 });

      if (!parsed || parsed.length === 0) {
        setError("PDF에서 유효한 데이터를 찾을 수 없습니다.");
        setPhase("upload");
        return;
      }

      const reviewRows: ReviewRow[] = parsed.map((item: { period: string; paymentAmount: number; vatAmount: number; totalAmount: number }, idx: number) => {
        const periodLabel = detectPeriodLabel(item.period);
        return {
          row: {
            id: `cc-${idx + 1}`,
            periodLabel,
  ...
  const totalPaymentAmount = selectedRows.reduce((s, r) => s + r.row.paymentAmount, 0);
  const totalVatAmount = selectedRows.reduce((s, r) => s + r.row.vatAmount, 0);
  const totalAmount = selectedRows.reduce((s, r) => s + r.row.totalAmount, 0);
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      const items = selected.map((r) => ({
        period: r.row.period,
        paymentAmount: r.row.paymentAmount,
        vatAmount: r.row.vatAmount,
    ...
    return (
      <div className="space-y-4">
        <div
          ...
          onClick={() => document.getElementById("credit-card-file-input")?.click()}
        >
          <Upload className="h-10 w-10 mx-auto text-muted-foreground mb-3" />
          <p className="text-lg font-medium mb-1">사업용신용카드 내역 업로드</p>
          <p className="text-sm text-muted-foreground mb-3">
            홈택스에서 다운로드한 사업용신용카드 PDF (.pdf)
          </p>
          <p className="text-xs text-muted-foreground">
            파일을 드래그하거나 클릭하여 업로드
          </p>
          <input
            id="credit-card-file-input"
            type="file"
            accept=".pdf"
        ...
        <div className="bg-muted/50 rounded-lg p-4 text-sm space-y-2">
          ...
          <ul className="list-disc list-inside space-y-1 text-muted-foreground">
            <li>PDF에서 AI를 활용하여 결제내역 자동 파싱</li>
            <li>기수(1기/2기) 자동 감지 (1~6월: 1기, 7~12월: 2기)</li>
            <li>사업용신용카드 테이블에 자동 등록</li>
            <li>매입 전표 자동 생성 (부가세 자동 분리)</li>
          </ul>
        </div>
      </div>
    );
  ...
  if (phase === "parsing") {
    return (
      <div className="flex flex-col items-center justify-center py-16 space-y-4">
        ...
        <p className="text-lg font-medium">AI가 PDF를 분석하고 있습니다...</p>
  ...
  if (phase === "saving") {
    return (
      <div className="flex flex-col items-center justify-center py-16 space-y-4">
        <Loader2 className="h-10 w-10 animate-spin text-primary" />
        <p className="text-lg font-medium">신용카드 내역 등록 및 전표 생성 중...</p>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center py-16 space-y-4">
        ...
        <div className="text-center text-muted-foreground space-y-1">
          <p>신용카드 내역 <strong className="text-primary">{result.created}</strong>건 등록, 전표 <strong className="text-primary">{result.journals}</strong>건 자동 생성</p>
        </div>
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium">{rows.length}건 신용카드 내역</span>
        </div>
      ...
      <div className="flex items-center justify-between bg-muted/50 rounded-lg p-3">
        <div className="flex items-center gap-3 text-sm">
          ...
          <span className="border-l pl-3">결제금액 <strong>{formatAmount(totalPaymentAmount)}</strong></span>
          <span>부가세 <strong>{formatAmount(totalVatAmount)}</strong></span>
          <span>합계 <strong>{formatAmount(totalAmount)}</strong></span>
        </div>
        <div className="flex gap-2">
            ...
            {filteredRows.map((row) => (
              ...
              >
                ...
                <TableCell className="text-right font-mono">
                  {formatAmount(row.row.paymentAmount)}
                </TableCell>
                <TableCell className="text-right font-mono text-muted-foreground">
                  {formatAmount(row.row.vatAmount)}
                </TableCell>
                <TableCell className="text-right font-mono font-medium">
                  {formatAmount(row.row.totalAmount)}
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
